...
def main(ckpt_dir: str, tokenizer_path: str, input_file: str = None, lr: float = 1e-3, num_epochs: int = 20, dataset: str = "gsm8k-xl", log_prefix="", only_functoken=False, log_each=False):
    ...
    # load the funcmodel from llama/model.py
    ...
    # only update tokens with gradients required (this ensures none of the base model params are updated as they are set to param.requires_grad = False)
    ...
    for epoch in range(num_epochs):
        ...
        for case_idx, prompt in tqdm(enumerate(trainset)):
            ...
            if (case_idx + 1) % 20 == 0: # every 20 train examples do
        ...
        text_file = open("debug_train.txt", "a+")
        for line in errors:
