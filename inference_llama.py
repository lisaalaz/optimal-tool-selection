...
from inference_modes_tool_choice import func_embedding_inference_tool_choice, kamel_embedding_inference_tool_choice, vh_embedding_inference_tool_choice
...
def load(ckpt_dir: str, tokenizer_path: str, local_rank: int, world_size: int, func_load_path: str, func_dict: dict) -> FunctionLM:
    ...
    model_args: ModelArgs = ModelArgs(max_seq_len=3000, max_batch_size=1, **params)
...
def main(ckpt_dir: str, tokenizer_path: str, temperature: float = 0, top_p: float = 0.95, mode: str = "baseline", dataset = "original",
    ...
    # During inference we need the templates with in-context exemplars to add args to the tool, so we load them
    templates = {}
    if dataset == "gsm8k-xl":
    ...
    for case_idx, question in tqdm(enumerate(test_cases), total=len(test_cases)):
        ...
        if mode == "func_embedding":
            ...
            if inference_style == "vanilla":
                ...
                log = vh_embedding_inference_tool_choice(case_idx, question, funcmodel, temperature, top_p, max_func_call)
            ...
            if inference_style == "vanilla":
                ...
                log = kamel_embedding_inference_tool_choice(templates, case_idx, question, funcmodel, temperature, top_p, max_gen_len, max_func_call)
